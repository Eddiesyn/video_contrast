...
class R2Plus1D_Unsupervised(nn.Module):
    def __init__(self, layer_sizes, low_dim, input_channel=3):
        super(R2Plus1D_Unsupervised, self).__init__()
    ...
    def forward(self, x):
        avg_f = self.feature(x)
        x_fc1 = self.fc1(avg_f)
        unormed_x = self.fc2(self.relu(x_fc1))
        f = F.normalize(unormed_x, p=2, dim=1)

        return avg_f, unormed_x, f
