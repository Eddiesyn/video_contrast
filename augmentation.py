...
class MultiScaleRandomCrop(object):
    def __init__(self, scales, size, consistent=True, interpolation=Image.BILINEAR):
        self.scales = scales
        self.size = size
        self.interpolation = interpolation
        self.consistent = consistent
    ...
    def __call__(self, imgmap):
        '''all imgs in imgmap has save spatial size'''
        img1 = imgmap[0]
        min_length = min(img1.size[0], img1.size[1])
        ...
        if self.consistent:
            ...
            imgs = [img.crop((x1, y1, x2, y2)) for img in imgmap]
            imgs = [img.resize((self.size, self.size), self.interpolation) for img in imgs]

            return imgs
        else:
            # should better not use this inconsistent spatial transform
            result = []
            for i in imgmap:
                scale = self.scales[random.randint(0, len(self.scales) - 1)]
                tl_x = random.random()
                tl_y = random.random()
                crop_size = int(min_length * scale)
                x1 = tl_x * (i.size[0] - crop_size)
                y1 = tl_y * (i.size[1] - crop_size)
                x2 = x1 + crop_size
                y2 = y1 + crop_size
